...
<?php include "../../../../block/header.php"; ?>
	...
	<div class="uitem">
		...
		<? if ($_GET['cours_menu']): ?>
			<!-- item header -->
			<?php include "../../../cours/item/iheader.php"; ?>
		<? endif ?>
		...
		<div class="uhwa">
			<div class="uhwa_l">
				...
				<? if (mysqli_num_rows($pack2) > 1): ?>
					<div class="uhwa_lp">
						<? while ($pack_d = mysqli_fetch_assoc($pack2)): ?>
							<a class="uhwa_lpi <?=($pack_id==$pack_d['id']?'uhwa_lpi_act':'')?> <?=($pack_d['home_work']!=1?'uhwa_lpi_de':'')?>" <?=($pack_d['home_work']==1?'href="/user/homework/admin/cours/?id='.$cours_id.'&pack_id='.$pack_d['id'].'"':'')?>>
								<span>Пакет:</span>
				...
				<div class="uhwa_c">
					<div class="uhwa_cb">
						<a class="uhwa_i" href="#/user/homework/admin/cours/list.php?id=<?=$cours_id?>">
							<div class="uhwa_itcn">Барлығы</div>
						</a>
						<a class="uhwa_i" href="/user/homework/admin/cours/list2.php?id=<?=$cours_id?>">
							...
							<div class="uhwa_cb">
								...
								<? if (mysqli_num_rows($lesson)): ?>
									<? while ($lesson_d = mysqli_fetch_assoc($lesson)): ?>
										<a class="uhwa_i <?=($lesson_d['home_work']!=1?'uhwa_i_de':'uhwa_i_sel')?>" href="/user/homework/admin/cours/list.php?id=<?=$cours_id?>&lesson_id=<?=$lesson_d['id']?>" data-cours-id="<?=$cours_id?>" data-lesson-id="<?=$lesson_d['id']?>">
