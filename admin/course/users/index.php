<? include "../icore.php";
	...
	// filter user all
	if ($_GET['on'] == 1) $cours_buy_all = db::query("select * from c_buy where cours_id = '$cours_id' and off is null");
	elseif ($_GET['off'] == 1) $cours_buy_all = db::query("select * from c_buy where cours_id = '$cours_id' and off is not null");
	...
	// filter cours
	if ($_GET['on'] == 1) $cours_buy = db::query("select * from c_buy where cours_id = '$cours_id' and off is null order by ins_dt desc limit $page_start, $page_age");
	elseif ($_GET['off'] == 1) $cours_buy = db::query("select * from c_buy where cours_id = '$cours_id' and off is not null order by ins_dt desc limit $page_start, $page_age");
	...
	<div class="uitem">
		<div class="bl_c">
			...
			<div class="uc_u">
				...
				<div class="uc_uc">
					<? if (mysqli_num_rows($cours_buy) != 0): ?>
				...
				<div class="form_c">
					...
					<? else: $pack_d = mysqli_fetch_assoc($pack); ?>
						<div class="form_im form_sel dsp_n">
							<div class="form_txt sel_clc pack" data-val="<?=$pack_d['id']?>"><?=$pack_d['name_'.$lang]?></div>
						</div>
					<? endif ?>
				</div>
