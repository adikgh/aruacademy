...
$(document).ready(function() {
	...
	$('.rad3').on('click', function () {
      ...
      ball = $('.sw_tb').attr('data-ball')
		...
		$.ajax({
			url: "/user/cours/tk/get.php?test_answer_tk1",
			type: "POST",
			dataType: "html",
			data: ({
				v: v, n: n, ball: ball,
	...
	$('.tk_t2_start').click(function (e) { 
		...
		$.ajax({
			url: "/user/cours/tk/get.php?tk_test_start",
			type: "POST",
			dataType: "html",
			data: ({
	...
	$('.rad4').on('click', function () {
      ...
      ball = $('.sw_tb').attr('data-ball-v1')
		...
		test_item_id = $(this).parent().attr('data-test-item-id')
		$.ajax({
			url: "/user/cours/tk/get.php?test_answer_tk3",
			type: "POST",
			dataType: "html",
			data: ({
				v: v, n: n, ball: ball,
	...
	$('.sw_tb').click(function () { 
		if (kt_t2.realIndex == $('.sw_tb').attr('data-number')) {
			...
			$.ajax({
				url: "/user/cours/tk/get.php?tk3_test_end",
				type: "POST",
				dataType: "html",
				data: ({ n: n }),
				success: function(data){
					if (data == 'yes') console.log(data);
					else $(location).attr('href', '/user/cours/tk/item3_t2.php?id=' + $('.sw_tb').attr('data-id') + '&test_id=' + data);
				},
				beforeSend: function(){ },
				error: function(data){ }
			})
	...
	$('.sw_tb').click(function () {
		if (kt_t4.realIndex == $('.sw_tb').attr('data-number')) {
			n = $('.sw_tb').attr('data-test-number')
			$.ajax({
				url: "/user/cours/tk/get.php?tk4_test_end",
				type: "POST",
				dataType: "html",
				data: ({ n: n }),
				success: function(data){
					if (data == 'yes') console.log(data);
					else $(location).attr('href', '/user/cours/tk/item34t.php?id=' + $('.sw_tb').attr('data-id') + '&test_id=' + data);
				},
				beforeSend: function(){ },
				error: function(data){ }
			})
